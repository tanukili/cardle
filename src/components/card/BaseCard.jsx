...
export default function BaseCard({
  ...
  const selectStyle = [isSelectMode && 'select-mode', isSelected && 'is-selected'].filter(Boolean).join(' ');
  ...
  const CardHeader = () => {
    ...
    return (
      <div className={`card-header p-lg-6 ${titleOnlyClass.header}`}>
        <h3 className={`fs-m text-center lh-base tracking-2 ${titleOnlyClass.title}`}>{card.title}</h3>
      </div>
    );
  };
    ...
    return (
      <>
        <ReactMarkdown
          ...
          components={{
            ul: ({ node, ...props }) => <ul className="list-unstyled card-ul-style mb-0" {...props} />,
  ...
  const BadgeContent = ({ cardBox }) => {
    return (
      <>
        <span className={`badge badge-${cardBox.color} lh-base mb-4`}>{cardBox.title}</span>
