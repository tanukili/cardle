...
const getModalCopy = (action, currentPlan, targetPlan, activeOrderNextBillingDateText) => {
  if (!action) return;
    ...
    return {
      ...
      description: (
        <>
          ...
          。
          <br />
          點擊 <span className="text-primary">查看方案明細</span> 前往方案明細頁。
        </>
      ),
      confirmText: '查看方案明細',
      cancelText: '關閉',
...
export default function PersonalPlanSwiper() {
  ...
  const [currentPlanId, setCurrentPlanId] = useState(userPlan?.id || 'plan_free');
  ...
  const targetPlan = useMemo(() => plans.find((p) => p.id === targetPlanId) || null, [plans, targetPlanId]);

  const modalCopy = useMemo(() => {
    return getModalCopy(action, userPlan, targetPlan, activeOrderNextBillingDateText);
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      if (action.policy === 'requires_payment') {
        setAfterClose(() => () => navigate(`/subscription/checkout/${targetPlanId}`));
        closeModal();
        return;
      }
    } catch (error) {
        ...
        {plans.map((plan, index) => {
          ...
          return (
            <SwiperSlide key={plan.id} className="swiper-slide h-auto">
              <div
                className={`bg-gray-0 p-6 border ${
                  isCurrent ? 'border-2 border-primary' : 'border-gray-100'
                } rounded-4 d-flex flex-column h-100`}
              >
                <div>
                  {/* 方案名稱 */}
                  <div className="pb-6 border-bottom border-gray-400 mb-8">
                    <div className="d-flex justify-content-between mb-6">
                      <span
                        className={`badge ${
                          plan.title === 'Free' ? 'text-gray-700' : 'bg-secondary text-gray-0'
                        } border border-gray-500 px-md-4 fs-s fs-md-l`}
                      >
                        {plan.title}
                      </span>
                      <span className="fs-s fs-md-l fw-bold text-gray-500">{plan.subtitle}</span>
                    </div>
                    <h3>
                      <span className="text-gray-1000 fs-4xl fs-md-5xl">NT$ {plan.price}</span>
                      <span className="text-gray-500 lh-base fw-normal fs-m fs-md-xl">/ {plan.billing?.unit}</span>
                    </h3>
                  </div>
                  {/* 方案內容 */}
                  <ul className="list-unstyled mb-8">
                    {plan.featuresShort.map((feature, index) => (
                      <li key={index} className="mb-3">
                        <span
                          className={`material-symbols-outlined align-bottom ${
                            plan.title === 'Free' ? 'text-gray-500' : 'text-secondary'
                          } me-4`}
                        >
                          check
                        </span>
