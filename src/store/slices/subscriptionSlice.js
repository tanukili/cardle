...
const pickActiveOrder = (orders) => {
  ...
  const candidates = orders.filter((order) => order.status === 'active' && Number(order.nextBillingDate) >= now);

  if (candidates.length === 0) return null;

  candidates.sort((a, b) => Number(b.nextBillingDate) - Number(a.nextBillingDate));
...
export const getUserSubscription = createAsyncThunk('subscription/getUserSubscription', async (_, thunkAPI) => {
  try {
    ...
    // 會員訂閱方案 (plans)
    const plan = activeOrder ? plans.find((p) => p.id === activeOrder.planId) || null : null;

    // 付款方式 (payment methods)
    const paymentMethod = activeOrder
      ? paymentMethods.find((pm) => pm.id === activeOrder.paymentMethodId) || null
      : null;
...
// Paid -> Free 停止續訂
export const switchPaidToFree = createAsyncThunk('subscription/paidToFree', async (_, thunkAPI) => {
  try {
    const state = thunkAPI.getState();
    const activeOrder = state.subscription.activeOrder;

    if (!activeOrder) return thunkAPI.rejectWithValue('目前沒有可變更的有效訂單');
...
// 年 -> 月
export const switchYearToMonth = createAsyncThunk('subscription/switchYearToMonth', async (_, thunkAPI) => {
  try {
    const state = thunkAPI.getState();
    const userId = state.user.userInfo.id;
...
// 月 -> 年
export const upgradeMonthToYear = createAsyncThunk('subscription/upgradeMonthToYear', async (_, thunkAPI) => {
  try {
    const state = thunkAPI.getState();
    const userId = state.user.userInfo.id;
    const { activeOrder, paymentMethod } = state.subscription;
